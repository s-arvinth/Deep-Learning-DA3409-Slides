...
\graphicspath{{figs_official/}{Class17_BiasVariance_DoubleDescent/A_Textbook_Figures/figs_official/}}
...
\begin{frame}{Where the error comes from}
  \begin{center}
    \includegraphics[width=0.95\linewidth]{PerfNoiseBiasVariance}
  \end{center}
  \figcap[0.97]{(a) noise: the targets scatter about the true function,
  leaving a floor no model can cross. (b) bias: the best available fit
  still departs from it. (c) variance: a fit to a finite sample departs
  again, differently each time. Prince (2023), Fig.~8.5.}
\end{frame}
...
\begin{frame}{Two ways to spend capacity}
  \vspace{0.25em}
  \begin{center}
    \includegraphics[height=0.295\textheight]{Bishop4_7a}\hspace{0.5em}%
    \includegraphics[height=0.295\textheight]{Bishop4_7c}\hspace{0.5em}%
    \includegraphics[height=0.295\textheight]{Bishop4_7e}%
  \end{center}
  \figcap[0.97]{Twenty of one hundred fits, each to its own sample of
  $N = 25$ points, at three values of the penalty:
  $\ln\lambda = 3$, $1$ and $-3$. The spread across the red curves is
  the variance. Bishop \& Bishop (2024), Fig.~4.7.}
\end{frame}

\begin{frame}{The trade-off, measured}
  \vspace{0.25em}
  \begin{center}
    \includegraphics[height=0.415\textheight]{Bishop4_8}\hspace{1.6em}%
    \includegraphics[height=0.415\textheight]{PerfBiasVarianceTradeoff}%
  \end{center}
  \figcap[0.97]{The same statement in both books. Left: against the
  penalty $\ln\lambda$, with the test error overlaid; the minimum of
  bias$^2 +$ variance sits where the test error is smallest.
  Bishop \& Bishop (2024), Fig.~4.8. Right: against model capacity.
  Prince (2023), Fig.~8.9.}
\end{frame}
...
\begin{frame}{What is actually measured}
  \begin{center}
    \includegraphics[height=0.495\textheight]{Bishop9_9}
  \end{center}
  \figcap[0.97]{ResNet18 on an image classification task; the horizontal
  axis is a width parameter governing the number of weights. Training
  error falls monotonically; test error falls, rises, and falls again,
  continuing to fall after the training error has reached zero.
  Bishop \& Bishop (2024), Fig.~9.9, from Nakkiran et al.\ (2019).}
\end{frame}
...
\begin{frame}{Reading the curve}
  ...
  \begin{itemize}\itemsep0.34em
    \item \textbf{Axes.} Capacity is a width parameter setting the
          number of parameters; the training set is fixed. Dashed is training error, solid is error
          on unseen data, and only the solid one matters
    \item \textbf{Threshold.} The model can fit every training
          point exactly; the training error reaches zero and stays there
    \item \textbf{Classical regime, left of the band.} Test error falls as bias
          falls, then rises as variance rises: the U-curve, with its
          \alert{classical minimum}
    \item \textbf{Critical regime, the band.} Exactly one interpolant
          exists and it is enormous; the test error peaks
    \item \textbf{Modern regime, right of the band.} Many interpolants exist and the
          smallest is chosen. Test error descends again to the
          \alert{modern minimum} --- below the classical one,
...
\begin{frame}{The same shape across four data sets}
  \begin{center}
    \includegraphics[height=0.50\textheight]{PerfDoubleDescent}
  \end{center}
  \figcap[0.97]{(a) MNIST-1D without label noise: no peak.
  (b) the same with $15\%$ of labels randomised: the peak appears.
  (c) MNIST with a shallow network. (d) CIFAR-100 with ResNet18. How
  pronounced the peak is depends on the noise in the labels.
  Prince (2023), Fig.~8.10.}
\end{frame}

\begin{frame}{Smoothness is what extra capacity buys}
  \begin{center}
    \includegraphics[height=0.50\textheight]{PerfSmoothness}
  \end{center}
  \figcap[0.97]{The smoothest function that still passes through every
  training point, as hidden units are added. With just enough capacity
  the curve must contort itself; with more, it can interpolate smoothly
  across the gap. Prince (2023), Fig.~8.11.}
\end{frame}
...
\begin{frame}{Capacity is not the parameter count}
  \begin{center}
    \includegraphics[height=0.525\textheight]{Bishop9_10}
  \end{center}
  \figcap[0.97]{Test error against training epochs for three model
  sizes. Effective complexity grows with the number of epochs, so the
  large model traverses the same three regimes in time that the previous
  figure traversed in width. Bishop \& Bishop (2024), Fig.~9.10, from
  Nakkiran et al.\ (2019).}
\end{frame}
...
\begin{frame}{More data can make a model worse}
  \begin{center}
    \includegraphics[height=0.525\textheight]{Bishop9_11}
  \end{center}
  \figcap[0.97]{Test loss for a transformer against embedding dimension,
  at two training set sizes. The larger set is better overall, but over
  the range marked by the arrows $4.5\times$ more data gives a
  \emph{worse} model. Bishop \& Bishop (2024), Fig.~9.11, from
  Nakkiran et al.\ (2019).}
\end{frame}
