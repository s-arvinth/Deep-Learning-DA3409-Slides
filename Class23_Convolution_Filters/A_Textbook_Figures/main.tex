...
%  and cited on the slide.  See _shared/PLAN_Classes23_24_25.md.
%
...
\graphicspath{{figs_official/}{Class23_Convolution_Filters/A_Textbook_Figures/figs_official/}}
...
\begin{frame}{The same statement, drawn}
  \begin{center}
    \includegraphics[height=0.505\textheight]{P_Conv2a}
  \end{center}
  \figcap[0.97]{(a--b) a fully connected layer on six inputs: every one
  of the $36$ weights is free. (c--d) a convolutional layer with kernel
  three: most entries are zero and the rest repeat, so three numbers
  describe the whole matrix. (e--f) the same with stride two.
  Prince (2023), Fig.~10.4.}
\end{frame}
...
\begin{frame}{Proof, in both directions}
  ...
  \begin{itemize}\itemsep0.28em
    \item \focus{1}{Forward. Write $y_i = (T_a \mathbf{x})_i = x_{i-a}$. Then
          ...
          = \sum_j w_j\, x_{i-a+j}$.}
    \item \focus{2}{That last expression is exactly
          $\mathrm{conv}(\mathbf{x})_{i-a}$, which is
          $T_a\,\mathrm{conv}(\mathbf{x})$ evaluated at $i$. \hfill $\square$}
    \item \focus{3}{Converse. Commuting with $T_a$ means
          $\bm{\Omega} T_a = T_a \bm{\Omega}$ for every $a$, and in
          coordinates that reads
          $\Omega_{i,\,j-a} = \Omega_{i-a,\,j}$.}
    \item \focus{4}{Setting $a = j$ gives
          $\Omega_{i,0} = \Omega_{i-j,\,j}$, so every entry is fixed by
          the single index $i-j$.}
    \item \focus{5}{Define $w_{i-j} = \Omega_{ij}$. Then
...
\begin{frame}{Equivariance, measured}
  \begin{center}
...
\begin{frame}{A convolution is a dense layer with most of it deleted}
  \vspace{0.3em}
  \begin{center}
    \includegraphics[height=0.40\textheight]{Bishop10_2}
  \end{center}
  \figcap[0.97]{A one-dimensional input with a kernel of width two. The
  connections are \alert{sparse}, because each unit reads only two
  inputs, and they are \alert{shared}, because links drawn in the same
  colour carry the same weight.
  Bishop \& Bishop (2024), Fig.~10.2.}
\end{frame}
...
\begin{frame}{The three knobs, drawn}
  \begin{center}
    \includegraphics[width=0.74\linewidth]{P_Conv1a}
  \end{center}
  \figcap[0.97]{(a--b) stride two evaluates the kernel at every other
  position. (c) a kernel of five reads more and costs more weights.
  (d) a dilated kernel reaches as far as a kernel of five while paying
  for three. Prince (2023), Fig.~10.3.}
\end{frame}
...
\begin{frame}{The recursion, checked}
  \begin{center}
    \includegraphics[height=0.51\textheight]{P_ConvNetworkRF}
  \end{center}
  \figcap[0.97]{(a) units in the first hidden layer see three inputs.
  (b) units in the second see five, because each of their three inputs
  saw three. (c) a third layer with stride two reaches seven. (d) by the
  fourth the field covers the whole input.
  Prince (2023), Fig.~10.6.}
\end{frame}
...
\begin{frame}{Resolution, measured}
  \vspace{0.2em}
  \begin{center}
    \includegraphics[width=0.455\linewidth]{P_ConvDown}\hspace{0.8em}%
    \includegraphics[width=0.455\linewidth]{P_ConvUp}%
  \end{center}
  \figcap[0.97]{Left: three ways down --- sub-sampling, max pooling and
  mean pooling. Right: three ways back up --- duplication, max
  unpooling using the remembered positions, and bilinear interpolation.
  Prince (2023), Figs.~10.11 and 10.12.}
\end{frame}
