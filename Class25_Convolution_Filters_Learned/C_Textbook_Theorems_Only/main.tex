...
%
%  C variant: the B deck with the three results absent from both set
%  texts removed -- the exact composition of two linear kernels, the
%  linear amplification epsilon ||w||_1 behind the fast gradient sign
%  method, and the translation invariance of the style matrix.  Bishop's
%  own statements stay in their place.  See _shared/PLAN_Classes23_24_25.md.
%
...
\graphicspath{{figs/}{Class25_Convolution_Filters_Learned/C_Textbook_Theorems_Only/figs/}}
...
\begin{frame}{Reach grows with depth}
  \begin{center}
    \includegraphics[height=0.60\textheight]{receptive_field}
  \end{center}
  \vspace{-0.5em}
  {\tiny\color{mcMuted}
  Three layers with a kernel of three at stride one. The maroon unit at
  the top of the output layer reads three units of the middle layer,
  each of which reads three of the input, so it depends on five inputs;
  the stack of small kernels reaches as far as one large kernel for
  fewer parameters, and is constrained to filters that factor.
  \hfill Bishop \& Bishop (2024), Fig.~10.9.\par}
\end{frame}

\begin{frame}{Small filters, composed}
  \vspace{0.1em}
  {\footnotesize
  \begin{itemize}\itemsep0.30em
    \item Earlier networks had fewer convolutional layers with larger
          receptive fields: AlexNet reads $11 \times 11$ at stride $4$
    \item The same reach can be had \emph{implicitly} from several
          layers of small kernels, for far fewer parameters
    \item Bishop reads this as an \alert{inductive bias}: the large
          filter must be composed of convolutional sub-filters, and
          only such large filters are available to the network
    \item With a ReLU between the layers the stack is not any single
          linear filter, which is what makes depth more than a
          factorisation --- the point of the next figure
  \end{itemize}}
  \vspace{0.3em}
  \begin{redhighlight}
    Depth buys reach cheaply, and it constrains what the reach is used
    for.
  \end{redhighlight}
  \srcline{Bishop \& Bishop (2024), \S10.2.8, Fig.~10.9.}
\end{frame}

\begin{frame}{Composition, computed}
  \begin{center}
    \includegraphics[height=0.62\textheight]{composition}
  \end{center}
  \vspace{-0.6em}
  {\tiny\color{mcMuted}
  A vertical-edge kernel followed by a blur, and the single
  $5 \times 5$ kernel equal to the pair; applying the pair or the one
  gives the same map to machine precision. With a ReLU between the
  two, the result differs from the single kernel: the difference has an
  RMS of $0.068$ against a map RMS of $0.088$.
  \hfill Bishop \& Bishop (2024), \S10.2.8.\par}
\end{frame}
...
\begin{frame}{VGG-16, drawn}
  ...
  $103$ million in the first fully connected layer, while most of the
  connections are convolutional. Bishop \& Bishop (2024), Fig.~10.10.}
\end{frame}
...
\begin{frame}{Gabor filters, drawn}
  \begin{center}
    \includegraphics[height=0.62\textheight]{gabor}
  \end{center}
  \vspace{-0.5em}
  {\tiny\color{mcMuted}
  Eq.~10.6 on a grid: the orientation $\theta$ runs from $0$ in the
  top row to $\pi/2$ in the bottom row, the frequency $\omega$ from $1$
  in the left column to $10$ in the right.
  \hfill Bishop \& Bishop (2024), Fig.~10.11.\par}
\end{frame}
...
\begin{frame}{Grad-CAM, computed by hand}
  \begin{center}
    \includegraphics[width=0.96\linewidth]{gradcam}
  \end{center}
  \vspace{-0.6em}
  {\tiny\color{mcMuted}
  A two-layer network small enough to differentiate on paper: Class
  23's four kernels with a ReLU, then one $3 \times 3$ kernel per channel
  summed into a class map, whose sum is the score. (b) the four maps
  and their $\alpha_k$, exact averages of a gradient written out as a
  ReLU mask sent back through each second-layer kernel. (c) $\mathbf{L}$
  over the image: the score depends most on horizontal edges.
  \hfill Bishop \& Bishop (2024), Eqs.~10.9--10.10.\par}
\end{frame}
...
\begin{frame}{Turning the gradient against the network}
  ...
  \begin{itemize}\itemsep0.18em
    \item Training moves the parameters to reduce $E$; the attack
          moves the image to increase it, with the same backpropagation
    \item A small $\epsilon$ keeps the change invisible, and yet the
          network misclassifies with high confidence
    \item Not over-fitting: much less flexible \alert{linear} models
          are fooled the same way
  \end{itemize}}
  \vspace{-0.3em}
...
\begin{frame}{The attack, run}
  \begin{center}
    \includegraphics[width=0.94\linewidth]{fgsm}
  \end{center}
  \vspace{-0.6em}
  {\tiny\color{mcMuted}
  A logistic classifier on raw pixels, trained to tell a disc from a
  square in noise of standard deviation $0.25$: $99\%$ on held-out
  images. (a) one test image, the sign perturbation at
  $\epsilon = 0.05$ --- a fifth of the noise already there --- and the
  result: $p(\mathrm{square})$ falls from $0.98$ to $0.46$. (b) accuracy
  against $\epsilon$, to chance by $0.05$ --- a linear model, fooled
  the same way.
  \hfill Bishop \& Bishop (2024), Eq.~10.11.\par}
\end{frame}
...
\begin{frame}{Why the style matrix forgets position}
  ...
  {\footnotesize
  \begin{itemize}\itemsep0.30em
    \item The content error matches features at the \emph{same
          locations} in $G$ and $C$
    \item Style is a different thing: if vertical edges in $S$ tend to
          come with orange blobs, the same should hold in $G$ ---
          \alert{wherever} the two occur. So the style error averages
          over locations, through the co-occurrence matrix $F_{kk'}$
    \item Early layers match low-level features such as edges; later
          layers match larger structures. The layer chosen for the
          content term decides how literal the copy is
    \item Several layers are combined as $\sum_l \lambda_l
          E^{(l)}_{\mathrm{style}}$, with the $\lambda_l$ set by eye
  \end{itemize}}
  \vspace{0.3em}
  \begin{redhighlight}
    Content is where the features are; style is which features go
    together.
  \end{redhighlight}
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.22em
    ...
    \item A stack of small kernels reaches as far as one large kernel
          for fewer parameters, and is constrained to filters that
          factor
