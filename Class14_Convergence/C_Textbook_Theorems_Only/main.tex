...
%
%  This is the textbook-only variant: every result stated here appears
%  in Bishop & Bishop (2024) or Prince (2023).  Five results that the
%  B variant states from the research literature -- the optimal fixed
%  learning rate, the heavy-ball rate, Nesterov's accelerated rate, the
%  Robbins-Monro conditions, and the step count in terms of kappa -- are
%  removed, and the discussion that replaces them is unquantified.
%
...
\graphicspath{{figs/}{Class14_Convergence/C_Textbook_Theorems_Only/figs/}}
...
\begin{frame}{One learning rate, many curvatures}
  ...
  one for \emph{every} eigenvalue, so the largest one binds.}
\end{frame}
...
\begin{frame}{When every direction curves alike}
  ...
  \end{center}
  \figcap[0.98]{With $\lambda_1 = \lambda_2$ the contours are circles,
...
\begin{frame}{What ill-conditioning costs}
  ...
  \end{center}
  \figcap[0.97]{(a) one contour of the same bowl at four condition
  numbers. (b) steps needed to reduce the error by six orders of
...
\begin{frame}{The valley}
  ...
  oscillate. (b) three fixed learning rates on that surface: none of
  them is fast.}
\end{frame}
...
\begin{frame}{Momentum on the valley}
  \begin{center}
    \includegraphics[height=0.455\textheight]{momentum_paths}
  \end{center}
  \figcap[0.97]{The same surface and start for all three, each run with
  the constants that are optimal for it. Left to right, the stride along
  the valley lengthens and the crossing of the valley disappears; the
  first twenty iterates are marked.}
\end{frame}
...
\begin{frame}{Five update rules, by family}
  \begin{center}
    \includegraphics[height=0.45\textheight]{optimiser_compare}
  \end{center}
  \figcap[0.97]{The same ill-conditioned quadratic and start throughout,
  each method at constants that suit it. (a) the momentum family
  attacks the ratio $\lambda_{\max}/\lambda_{\min}$. (b) the adaptive
  family rescales each coordinate instead. (c) all five by error. Why
  the families differ matters more than the ordering on any one
  surface.}
\end{frame}
...
\begin{frame}{What each method actually fixes}
  ...
  \begin{tabular}{@{}l@{\hspace{1.3em}}l@{\hspace{1.3em}}l@{}}
    \textbf{Method} & \textbf{Attacks} & \textbf{How}\\
    gradient descent & --- & one rate for every direction\\
    momentum & the condition number & inertia along a consistent slope\\
    Nesterov momentum & the condition number & the gradient at the
      predicted point\\
    schedules & the gradient noise & $\eta$ decays with the step index\\
    RMSProp, Adam & unequal curvature per parameter & a rate per
      parameter\\
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.30em
    ...
    \item The condition number $\kappa$ measures the gap, and it is what
          makes progress along the long axis slow
    \item Momentum accelerates along a consistent slope by $1/(1-\mu)$
          and cancels across an oscillating one
          (Proposition~\ref{th:mom})
    \item A fixed $\eta$ leaves a noise floor, and a decaying schedule
          removes it
