...
| `fig07_momentum_paths` | `momentum_paths` | descent, momentum and Nesterov on the same valley, **one method per panel**, each at its optimal constants |
...
| `fig11_optimiser_compare` | `optimiser_compare` | five update rules on one ill-conditioned quadratic, split by family, with a shared error panel |
