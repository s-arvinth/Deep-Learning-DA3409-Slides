...
\begin{frame}{Why not just use fixed basis functions?}
  \bridge{Before accepting learned features as progress, see what
  goes wrong without them.}
  Tile the input space and put one basis function in each cell
  ...
  \begin{propx}[The grid blows up]
    {\tiny Partition each of the $D$ axes into $S$ intervals. The
    tensor-product grid then has \alert{$S^{D}$} cells, so the model has
    $S^{D}$ parameters --- exponential in the input dimension.}
  \end{propx}
  \vspace{0.1em}
  {\tiny
  \emph{Proof.} A cell is a choice of one interval on each axis
  independently, so the number of cells is the size of
  $\{1,\dots,S\}^{D}$, namely $S^{D}$. \hfill$\square$}
  \vspace{-0.3em}
  {\scriptsize With $S=10$ and a $32\times32$ greyscale image
  ($D=1024$): $10^{1024}$ cells.
  \hfill Cf.\ Bishop \& Bishop (2024), Fig.~6.2.}
\end{frame}
...
\begin{frame}{The classical statement}
  ...
  \begin{theorem}[Universal approximation; Cybenko 1989, Hornik 1991,
                  Leshno et al.\ 1993]
    {\tiny Let $h:\mathbb{R}\to\mathbb{R}$ be continuous. The family
    \[
      \mathcal{N}_{1,\infty}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \item ``Only if'': a degree-$p$ activation traps every output in the
          finite-dimensional space of degree-$\le p$ polynomials
...
\begin{frame}{Two objects the next two theorems need}
  ...
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.485\textwidth}
      ...
      \end{block}
      {\tiny A smooth bump: small $C_f$. A sawtooth with $2^{L}$
      teeth: $C_f$ grows like $2^{L}$.}
    \end{column}
    \hfill
    \begin{column}{0.485\textwidth}
      ...
      \end{block}
      {\scriptsize A bound over a ball is \alert{worst-case}: it must
      hold for the nastiest member.}
    \end{column}
  \end{columns}
  \vspace{-0.1em}
  \begin{redhighlight}
    {\scriptsize One measures a \alert{single} function's wiggliness;
    the other describes a \alert{whole class} --- which is why the two
    bounds point in opposite directions.}
  \end{redhighlight}
\end{frame}
...
\begin{frame}{A rate that does not see the dimension}
  ...
  \begin{theorem}[Barron, 1993]
    {\footnotesize Let $f:\mathcal{K}\to\mathbb{R}$ have finite Barron norm
...
\begin{frame}{And a lower bound that does}
  ...
  \begin{theorem}[DeVore, Howard \& Micchelli, 1989]
    {\tiny Let $\mathcal{F}$ be the unit ball of the Sobolev space
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \item This is not a statement about networks in particular --- it
          limits \alert{every} scheme with continuous parameter
          selection
    \item No contradiction with Barron: his class is \alert{much
          smaller} than a Sobolev ball, and its norm $C_f$ can itself
          grow with $D$
  \end{itemize}}
  \vspace{-0.1em}
  \begin{redhighlight}
...
\begin{frame}{Width as a threshold}
  ...
  \begin{theorem}[Width-bounded universality; Lu, Pu, Wang, Hu \& Wang,
                  2017]
    {\tiny ReLU networks of width $D+4$ and \alert{arbitrary depth} are
  ...
  \begin{theorem}[Width lower bound; Hanin \& Sellke 2017, Johnson 2019]
    {\tiny ReLU networks of width \alert{$\leq D$} are \emph{not}
    universal, however deep. There are continuous functions they cannot
    approximate to better than a fixed constant.}
  \end{theorem}
  \vspace{-0.1em}
  {\tiny
  \emph{Why the lower bound holds.} A ReLU network of width $\leq D$
  never expands the space it maps through, and the consequence is that
  all of its \alert{level sets are unbounded}. A function with a bounded
  level set --- for instance $\lVert\mathbf{x}\rVert$, whose level sets
  are spheres --- therefore cannot be approximated.}
  \vspace{-0.2em}
  \begin{redhighlight}
    {\scriptsize \alert{Width is a threshold}; \alert{depth is a
    resource}.}
  \end{redhighlight}
\end{frame}
...
\begin{frame}{The witness, recalled}
  ...
  \begin{lemmax}[The sawtooth, from Class 7]
    {\scriptsize $T(x)=2h(x)-4h(x-\tfrac12)$ is a two-unit ReLU network that
...
\begin{frame}{The separation, measured}
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.53\textwidth}
      \centering
      \includegraphics[height=0.585\textheight]{depth_width}
    \end{column}
    \hfill
    \begin{column}{0.43\textwidth}
      \vspace{0.6em}
      {\footnotesize
      \begin{itemize}\setlength{\itemsep}{5pt}
        \item Target: $T^{\circ 3}$, eight linear pieces
        \item (a) $M=4$ cannot track the shape
        \item (b) $M=8$ has enough pieces, in the wrong places
        \item (c) $M=7$ with joints on the breakpoints is
              \alert{exact}
        \item (d) the error stays at order $1$ until $M=2^{3}-1$; the
              depth-$3$ network needs $19$ parameters
      \end{itemize}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{How deep networks beat the grid}
  ...
  costs on the smoothness ball.}
  \begin{theorem}[Yarotsky, 2017]
    {\tiny For $f$ in the unit ball of $C^{n}([0,1]^{D})$ and any
  ...
  {\tiny
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item The construction: deep ReLU networks approximate $x^{2}$, and
          hence products, to accuracy $\varepsilon$ with only
          $O(\log(1/\varepsilon))$ layers --- the sawtooth again
    \item Products give local Taylor polynomials, and a partition of
          unity glues them together
    \item Depth enters only \alert{logarithmically}; the
          $\varepsilon^{-D/n}$ is the DeVore lower bound, unavoidable
  \end{itemize}}
  \vspace{-0.1em}
  \begin{redhighlight}
...
\begin{frame}{Counting linear regions}
  ...
  \begin{theorem}[Lower bound; Mont\'ufar, Pascanu, Cho \& Bengio, 2014]
    {\tiny A ReLU network with $L$ hidden layers of width $M\geq D$ on
    $\mathbb{R}^{D}$ can realise at least
    $\big(\prod_{l=1}^{L-1}\lfloor M/D\rfloor^{D}\big)
    \sum_{j=0}^{D}\binom{M}{j}$ linear regions.}
  \end{theorem}
  \vspace{0.1em}
  \begin{theorem}[Upper bound; Serra, Tjandraatmadja \& Ramalingam, 2018]
    ...
    follow from the dimensions of the layers.}
  \end{theorem}
  \vspace{0.1em}
  {\footnotesize
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item Both are \alert{exponential in $L$}, \alert{polynomial in
          $M$} --- the same asymmetry as the sawtooth
    \item The trailing $\sum_{j\leq D}\binom{M}{j}$ is the
          \alert{Zaslavsky count} from Class 5: the last layer behaves
          like a shallow network
...
\begin{frame}{Capacity has a price}
  ...
  \begin{theorem}[VC dimension; Bartlett, Harvey, Liaw \& Mehrabian, 2019]
    {\tiny For ReLU networks with $W$ parameters and $L$ layers,
    \[
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item Learning theory then needs
          ...
          $\epsilon$
    \item The depth that buys expressive power also raises the
          worst-case sample requirement, \alert{linearly in $L$}
...
\begin{frame}{What none of them say}
  ...
  \begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item \alert{Architecture matters more than the counts suggest.}
          Convolution and attention change the function class, not just
          its size
  \end{itemize}
