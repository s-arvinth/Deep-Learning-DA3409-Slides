...
\graphicspath{{figs/}{Class27_Encoder_Decoder_Seq2Seq/C_Textbook_Theorems_Only/figs/}}
...
\begin{frame}{What teacher forcing leaves out}
  ...
  {\footnotesize
  \begin{itemize}\itemsep0.30em
    \item At inference the decoder uses its own estimated output
          $\hat y_t$ as the next input, so it tends to deviate more and
          more from the gold target as it keeps generating
    \item In training it was given the gold token instead, which speeds
          training but means the inputs it sees at test time differ from
          the ones it was trained on
    \item Goodfellow's remedy: mix free-running and teacher-forced
          inputs during training, so the network learns to handle its
          own errors
    \item The mismatch is visible two slides back: small losses
          on gold inputs, and a free-running output wrong from step six
  \end{itemize}}
  \srcline{Jurafsky \& Martin (2026), \S14.7.1; Goodfellow et al.\ (2016), \S10.2.1.}
\end{frame}
...
\begin{frame}{Where the context can be}
  ...
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.50\textwidth}
      ...
      {\footnotesize
      \begin{itemize}\itemsep0.28em
        \item The context is still a single vector, but now a weighted
              average of all the encoder hidden states, with weights that
              depend on the decoder's state
        \item It is generated anew at every decoding step, so it is
              different for each token produced
        \item On the reversal task the weights are nearly one-hot, so
              each $\mathbf{c}_i$ is, to a good approximation, one encoder
              state --- a different one each step
      \end{itemize}}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{center}
        \includegraphics[width=0.78\linewidth]{context_hull}
      \end{center}
      \vspace{-0.6em}
      {\tiny\color{mcMuted} Encoder states of one source, two principal
      directions; the contexts and the fixed context.\par}
    \end{column}
  \end{columns}
  \srcline{Jurafsky \& Martin (2026), \S14.8.}
\end{frame}
...
\begin{frame}{What the beam costs, and what it does not buy}
  ...
  {\footnotesize
  \begin{itemize}\itemsep0.30em
    \item At each step every prefix in the beam is extended by every
          token and only the $k$ best extensions are kept, so the work
          grows with the beam width and the sequence length, not with
          the number of possible sequences
    \item A prefix that falls out of the top $k$ is gone whatever its
          continuations would have scored, so the beam can still miss
          the most probable sequence; a wider beam misses less and costs
          more
    \item Longer sequences have more factors below one, so raw $\log p$
          favours short outputs; the standard fix normalises by length,
          $\frac{1}{m}\sum_t \log p(y_t \mid \cdot)$
    \item Minimum Bayes risk decoding picks, from the candidates, the
          one most similar to the rest --- one line, and beyond today
  \end{itemize}}
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.24em
    ...
    \item A fixed context is a bottleneck, and accuracy falls with source
          ...
          combination of the encoder states (Definition~\ref{th:attention})
    \item Greedy decoding is not the most probable sequence; beam search
          keeps $k$ prefixes and comes closer (Definition~\ref{th:beam})
  \end{enumerate}}
  \vspace{-0.2em}
