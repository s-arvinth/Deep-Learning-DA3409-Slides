...
\begin{frame}{The power method, drawn}
  \begin{center}
    \includegraphics[width=0.86\linewidth]{power_iteration}
  \end{center}
  \vspace{-0.7em}
  {\tiny\color{mcMuted}
  (a) $\|\mathbf{U}^t\mathbf{h}_0\|$ for a random $20 \times 20$ matrix
  rescaled to spectral radius $0.9$, $1.0$ and $1.1$: after $60$ steps
  the norm is $2 \times 10^{-4}$, $0.1$ and $29$. (b) a random
  linear--tanh layer composed $1$ to $8$ times, seen along one line
  through a $100$-dimensional state: flat almost everywhere, steep in a
  few places, and more so with every composition.
  \hfill Goodfellow et al.\ (2016), Eqs.~10.36--10.39, Fig.~10.15.\par}
\end{frame}
...
\begin{frame}{The gradient by lag, measured}
  \begin{center}
    \includegraphics[width=0.86\linewidth]{gradient_lag}
  \end{center}
  \vspace{-0.7em}
  {\tiny\color{mcMuted}
  $\|\partial L_T/\partial\mathbf{h}_{T-k}\|$ against the lag $k$ on
  length-$40$ sequences, read off the tape, for three cells of width
  $32$ trained here to recall the first symbol. (a) at initialisation
  all three fall. (b) after training the simple RNN's gradient at the
  first step is $10^{-30}$ of its value at the last, the LSTM's
  $10^{-2}$, the GRU's $0.4$: the gated cells have learned to carry it.
...
\begin{frame}{The LSTM cell, drawn}
  \vspace{-0.4em}
  \begin{center}
  \begin{tikzpicture}[>=Stealth, font=\small, x=1.25cm, y=1.0cm, scale=1, every node/.style={transform shape},
      gate/.style={circle, draw=dpc, fill=dpc!10, line width=0.9pt, minimum size=0.55cm, inner sep=0pt},
      act/.style={circle, draw=shc, fill=shc!10, line width=0.9pt, minimum size=0.55cm, inner sep=0pt},
      op/.style={circle, draw=tealc, fill=tealc!10, line width=0.9pt, minimum size=0.42cm, inner sep=0pt},
      ar/.style={->, line width=0.9pt}]
    % inputs on the left
    \node (c0) at (0, 2.6) {$\mathbf{c}_{t-1}$};
    \node (h0) at (0, 0.3) {$\mathbf{h}_{t-1}$};
    \node (x0) at (0, -0.7) {$\mathbf{x}_t$};
    % the four layers
    \node[gate] (f) at (2.6, 1.0) {$\sigma$};
    \node[act]  (g) at (3.9, 1.0) {$\tanh$};
    \node[gate] (i) at (5.2, 1.0) {$\sigma$};
    \node[gate] (o) at (7.4, 1.0) {$\sigma$};
    \foreach \n in {f,g,i,o}{
      \draw[ar, shc!70] (h0.east) -- (\n.south);
      \draw[ar, shc!70] (x0.east) -- (\n.south);
    }
    \node[dpc, above left=0pt and -1pt of f] {$\mathbf{f}_t$};
    \node[shc, above left=0pt and -1pt of g] {$\mathbf{g}_t$};
    \node[dpc, above right=0pt and -1pt of i] {$\mathbf{i}_t$};
    \node[dpc, above left=0pt and -1pt of o] {$\mathbf{o}_t$};
    % the cell line
    \node[op] (m1) at (2.6, 2.6) {$\odot$};
    \node[op] (m2) at (5.2, 1.85) {$\odot$};
    \node[op] (p1) at (4.55, 2.6) {$+$};
    \node[act, minimum size=0.5cm] (th) at (7.4, 2.6) {$\tanh$};
    \node[op] (m3) at (8.5, 2.6) {$\odot$};
    \node (c1) at (9.8, 2.6) {$\mathbf{c}_t$};
    \node (h1) at (9.8, 1.7) {$\mathbf{h}_t$};
    \draw[ar, tealc] (c0) -- (m1); \draw[ar, dpc] (f) -- (m1);
    \draw[ar, tealc] (m1) -- (p1);
    \draw[ar, shc] (g) -- (3.9, 1.85) -- (m2); \draw[ar, dpc] (i) -- (m2); \draw[ar, tealc] (m2) -- (5.2, 2.6) -- (p1);
    \draw[ar, tealc] (p1) -- (th);
    \draw[ar, tealc] (p1) -- (6.1, 2.6) -- (6.1, 3.25) -- (9.3, 3.25) -- (c1);
    \draw[ar, shc] (th) -- (m3); \draw[ar, dpc] (o) -- (m3);
    \draw[ar, shc] (m3) |- (h1);
    \node[mcMuted, anchor=west, font=\scriptsize] at (0.2, 3.65) {top line: the cell is multiplied by $\mathbf{f}_t$, added to, and passed on --- no weight matrix on this path};
  \end{tikzpicture}
  \end{center}
  \vspace{-0.5em}
  {\tiny\color{mcMuted} Inputs $\mathbf{c}_{t-1}$, $\mathbf{h}_{t-1}$, $\mathbf{x}_t$; outputs
  $\mathbf{c}_t$, $\mathbf{h}_t$. Gates in maroon, activations in purple, elementwise
  operations in teal. From outside, the only difference from a simple cell is the extra vector in and out.\par}
  \srcline{Jurafsky \& Martin (2026), Fig.~14.13; Goodfellow et al.\ (2016), Fig.~10.16.}
\end{frame}
...
\begin{frame}{What the gates do while remembering}
  \begin{center}
    \includegraphics[width=0.86\linewidth]{gate_activity}
  \end{center}
  \vspace{-0.7em}
  {\tiny\color{mcMuted}
  The LSTM trained here, on one length-$40$ sequence whose first symbol
  it must recall. (a) the forget gate of every unit against time: most
  units hold near one throughout, a few open and close with the input.
  (b) the mean forget gate stays at $0.86$; the mean input gate is
  highest at the first step, when the fact arrives.
  \hfill Jurafsky \& Martin (2026), \S14.5; Goodfellow et al.\ (2016), \S10.10.1.\par}
\end{frame}
...
\begin{frame}{Three cells on one task}
  ...
  \vspace{-0.7em}
  {\tiny\color{mcMuted}
  An RNN, an LSTM and a GRU of width $32$, trained the same way for
  $3{,}000$ steps to read $4$--$24$ symbols and emit the first. (a)
  training loss. (b) accuracy by sequence length, tested to $60$: the
  gated cells are perfect at every length, including far beyond what
  they trained on; the simple RNN never passes $81\%$, even at length
  $8$. Chance is $1/8$.
...
\begin{frame}{Clipping at a cliff, run}
  \begin{center}
    \includegraphics[width=0.84\linewidth]{clipping}
  \end{center}
  \vspace{-0.7em}
  {\tiny\color{mcMuted}
  A recurrent net with two parameters, $h_t = w\,h_{t-1} + b$ for $50$
  steps and a squared error at the end, so the loss has a cliff at
  $w = 1$. Gradient descent from the same start with the same step size:
  (a) without clipping the first step at the cliff throws the parameters
  across the plane, to a loss of $0.36$; (b) with the norm clipped at
  $v = 1$ the steps are bounded and settle at a loss of $0.005$.
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.24em
    ...
    \item Measured: the gated cells recall a fact across $60$ steps and
          carry the gradient with them; the simple RNN does neither
