...
\begin{frame}{The risk, in closed form}
  ...
  \begin{thmx}[Ridgeless least squares, isotropic design]\label{th:ridgeless}
    ...
    As $N, p \to \infty$ with $ \gamma:=p/N$ (Overparamterization ratio), the excess risk tends to
    \[
      R(\gamma) =
      \begin{cases}
...
\begin{frame}{Reading the two branches}
  ...
  \begin{itemize}\itemsep0.30em
    \item \focus{1}{Below the threshold there is one least-squares
          ...
          $\sigma^2\gamma/(1-\gamma)$ grows as $\gamma\rightarrow 1$ .}
    \item \focus{1}{At $\gamma = 1$ the design is square and almost
          singular. Exactly one vector interpolates, and it is forced to
          be enormous.}
    \item \focus{1}{Above the threshold the interpolating set is an
          affine subspace of dimension $p - N$, and minimum norm picks
          one point of it.}
    \item \focus{1}{Growing $\gamma$ increases that subspace, so the
          chosen point can be smaller: the noise term
          $\sigma^2/(\gamma-1)$ falls.}
    \item \focus{1}{The price is the bias term
...
\begin{frame}{Theory against measurement}
 ...
 and on both sides of the pole. (b) the squared norm
  of the fitted vector, which peaks at exactly the same place.}
\end{frame}

\begin{frame}{Smoothness and Extra Capacity}
  \begin{center}
...
\end{document}
This intuition explains why model variance shrinks as the parameter-to-sample ratio $\gamma = p/N$ grows in the overparameterized regime ($\gamma > 1$).Geometric Subspace ExpansionWhen $p > N$, the system $\mathbf{X} \hat{\mathbf{w}} = \mathbf{Y}$ is underdetermined. The set of all parameter vectors that perfectly interpolate the training data forms an affine subspace of dimension $p - N$.Higher Dimensions, Larger Subspace: As $\gamma$ increases for a fixed $N$, $p$ grows larger, expanding the dimension $p - N = N(\gamma - 1)$ of this solution subspace.Shorter Distance to the Origin: The minimum-norm estimator $\hat{\mathbf{w}} = \mathbf{X}^+ \mathbf{Y}$ selects the single vector in this interpolating subspace that lies closest to the origin. Because higher-dimensional subspaces have more directions to pass near the origin, the Euclidean norm $\Vert{}\hat{\mathbf{w}}\Vert{}_2$ required to fit the data becomes smaller on average.Noise Dilution Across CoordinatesThe residual label noise $\mathbf{\varepsilon} \sim \mathcal{N}(0, \sigma^2 \mathbf{I}_N)$ is a fixed $N$-dimensional vector that must be perfectly fit by the model:$$\mathbf{X} \hat{\mathbf{w}} = \mathbf{X} \mathbf{w} + \mathbf{\varepsilon}$$Spreading the Noise: In an isotropic setting with $p \gg N$, the estimator spreads the responsibility of fitting this $N$-dimensional noise vector across all $p$ available orthogonal directions.Reduced Per-Coordinate Variance: Because the noise variance is shared over vastly more feature dimensions, each individual component of $\hat{\mathbf{w}}$ undergoes smaller random fluctuations from dataset to dataset.As $\gamma \to \infty$, the noise energy per parameter dimension vanishes, causing the integrated variance term $\frac{\sigma^2}{\gamma - 1}$ to drop toward zero.