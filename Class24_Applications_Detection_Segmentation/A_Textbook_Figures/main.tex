...
\graphicspath{{figs_official/}{Class24_Applications_Detection_Segmentation/A_Textbook_Figures/figs_official/}}
...
\begin{frame}{Scoring a box}
  ...
  \end{defx}
  \vspace{-0.3em}
  \begin{propx}[What the ratio buys]\label{th:iouprop}
    \small
    $0 \leq \mathrm{IoU} \leq 1$, with $\mathrm{IoU} = 1$ exactly when
    the boxes coincide; and IoU is unchanged when both boxes are
    rescaled by the same factor, so the same relative error scores the
    same on a small object and a large one.
  \end{propx}
  \vspace{-0.15em}
  {\footnotesize
  \begin{itemize}\itemsep0.1em
    \item Correct above a threshold, usually $0.5$. Hard to optimise by
          gradient descent, so it is an \alert{evaluation} metric
  \end{itemize}}
  \vspace{-0.3em}
...
\begin{frame}{Proof of the proposition}
  ...
  \begin{itemize}\itemsep0.30em
    \item \focus{1}{The intersection is contained in the union, so
          $0 \leq \mathrm{area}(\cap) \leq \mathrm{area}(\cup)$ and the
          ratio lies in $[0, 1]$.}
    \item \focus{2}{$\mathrm{IoU} = 1$ forces
          $\mathrm{area}(\cap) = \mathrm{area}(\cup)$; since
          $\mathrm{area}(\cup) = \mathrm{area}(\mathbf{b}) + \mathrm{area}(\mathbf{b}^\star)
          - \mathrm{area}(\cap)$, both boxes have the area of the
          intersection, so both equal it.}
    \item \focus{3}{Scaling every coordinate by $s$ scales every area
          by $s^2$, in the numerator and the denominator alike, so the
          ratio is unchanged.
          \hfill $\square$}
  \end{itemize}}
  \vspace{-0.1em}
  \begin{redhighlight}
    The metric is dimensionless on purpose: a box eight pixels off is
    a small error on a bus and a large one on a bird.
  \end{redhighlight}
\end{frame}

\begin{frame}{IoU, drawn}
  \begin{center}
    \includegraphics[width=0.62\linewidth]{Bishop10_20}
  \end{center}
  \figcap[0.97]{The predicted box in blue and the ground truth in red:
  the area of the intersection, shaded on the left, divided by the area
  of the union, shaded on the right. Bishop \& Bishop (2024),
  Fig.~10.20.}
\end{frame}
...
\begin{frame}{The saving, drawn}
  \vspace{-0.2em}
  \begin{center}
    \includegraphics[height=0.36\textheight]{Bishop10_22}\hspace{1.5em}%
    \includegraphics[height=0.44\textheight]{Bishop10_23}%
  \end{center}
  \vspace{-0.2em}
  \figcap[0.97]{Left: the toy network --- a $3 \times 3$ convolution,
  $2 \times 2$ pooling and one fully connected output --- on a
  $6 \times 6$ image. Right: the same network on an $8 \times 8$ image,
  every layer enlarged; the blue regions are the only additional
  computation. Bishop \& Bishop (2024), Figs.~10.22 and 10.23.}
\end{frame}
...
\begin{frame}{Unpooling, drawn}
  \vspace{-0.2em}
  \begin{center}
    \includegraphics[height=0.20\textheight]{Bishop10_28a}\hspace{2em}%
    \includegraphics[height=0.20\textheight]{Bishop10_28b}\\[0.6em]
    \includegraphics[height=0.20\textheight]{Bishop10_29}
  \end{center}
  \vspace{-0.2em}
  \figcap[0.97]{Top: (a) the analogue of average pooling copies each
  value into its block; (b) the analogue of max pooling puts it in the
  first cell. Bottom: remembering where each maximum was and putting
  the value back there. Bishop \& Bishop (2024), Figs.~10.28 and
  10.29.}
\end{frame}
...
\begin{frame}{Transpose convolution, drawn}
  \begin{center}
    \includegraphics[height=0.50\textheight]{Bishop10_30}
  \end{center}
  \vspace{-0.2em}
  \figcap[0.97]{A $3 \times 3$ filter with an output stride of two: the
  red output patch is the kernel times the red input unit, the blue
  patch likewise, and cells where patches overlap sum or average the
  contributions. Bishop \& Bishop (2024), Fig.~10.30.}
\end{frame}
...
\begin{frame}{What the bottleneck discards}
  \vspace{-0.1em}
  \begin{propx}[A bottleneck alone cannot place every pixel]\label{th:rank}
    \small
    A decoder that is linear in a bottleneck of $m$ units produces
    outputs of rank at most $m$. With $m = 7 \cdot 7 \cdot 512 = 25{,}088$
    and $224^2 = 50{,}176$ output positions, it cannot produce an
    arbitrary per-pixel labelling; the position of an edge must come
    from elsewhere.
  \end{propx}
  \vspace{0.3em}
  \begin{itemize}\itemsep0.26em
    \item Pooling five times leaves $49$ positions to describe
          $50{,}176$; whatever survives is \emph{what}, not
          \emph{where} to the pixel
    \item The remembered unpooling positions carry some of it across;
          the U-net carries all of it
  \end{itemize}
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.22em
    ...
    \item A box is scored by intersection over union, a dimensionless
          ratio (Definition~\ref{th:iou}, Proposition~\ref{th:iouprop})
    ...
    \item A bottleneck cannot place every pixel on its own
          (Proposition~\ref{th:rank}); the U-net hands the resolution
          across by concatenation
  \end{enumerate}}
