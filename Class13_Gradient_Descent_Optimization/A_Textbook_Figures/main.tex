  ...
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{tabular}{@{}l@{\hspace{1.5em}}l@{}}
        ...
        $\mathbf{w}$ & every learnable parameter\\
        $E(\mathbf{w})$ & the error function being minimized\\
        $E_n(\mathbf{w})$ & its contribution from point $n$\\
        $\nabla E$ & gradient of $E$ with respect to $\mathbf{w}$\\
        $\mathbf{H}$ & Hessian, $\nabla\nabla E$\\
      \end{tabular}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{tabular}{@{}l@{\hspace{1.5em}}l@{}}
        ...
        $\lambda_i$, $\mathbf{u}_i$ & eigenvalues, eigenvectors of $\mathbf{H}$\\
      \end{tabular}
    \end{column}
  \end{columns}}
\end{frame}
...
\begin{frame}{The error surface}
  ...
  \end{center}
  \figcap[0.97]{$E$ viewed as a surface over weight space. $\mathbf{w}_A$ is a
  local minimum and $\mathbf{w}_B$ the global minimum; at $\mathbf{w}_C$ the local slope
...
\begin{frame}{The hiker in fog}
  ...
  \begin{tabular}{@{}l@{\hspace{2.2em}}l@{}}
    ...
    the altitude at your feet & $E(\mathbf{w})$ at the current $\mathbf{w}$\\
...
\begin{frame}{Convexity}
  \begin{block}{Definition (convex function)}
    ...
    \[
      E\big(\theta \mathbf{u} + (1-\theta) \mathbf{v}\big)
        \;\leq\; \theta E(\mathbf{u}) + (1-\theta) E(\mathbf{v})
      \qquad \forall\, \mathbf{u}, \mathbf{v}, \; \theta \in [0,1].
    \]
  \end{block}
  \vspace{0.4em}
  \begin{itemize}\itemsep0.4em
    \item Where $E$ is twice differentiable this says the Hessian $\mathbf{H}$ is
...
\begin{frame}{What convexity buys}
  ...
  \begin{itemize}\itemsep0.5em
    ...
    \item Progress can be measured: $E(\mathbf{w}) - E(\hat{\mathbf{w}})$ is a meaningful
...
\begin{frame}{Gradient descent}
  ...
  \]
  and differs only in the choice of $\Delta \mathbf{w}$. Today,
  \vspace{-0.2em}
...
\begin{frame}{Why the step goes along $-\nabla E$}
  ...
  $\Delta E \simeq \Delta \mathbf{w}^{\mathsf T}\,\nabla E(\mathbf{w})$.
  \vspace{-0.15em}
  \begin{lemmax}[The negative gradient is the steepest direction]\label{th:steep}
    \small
    Fix $\mathbf{w}$ with $\nabla E(\mathbf{w}) \neq 0$. Among all unit vectors $\mathbf{d}$, the
    quantity $\mathbf{d}^{\mathsf T}\nabla E(\mathbf{w})$ is minimised uniquely at
    $\mathbf{d}^\star = -\nabla E(\mathbf{w}) / \|\nabla E(\mathbf{w})\|$, where it equals
    $-\|\nabla E(\mathbf{w})\|$.
  \end{lemmax}
  \vspace{0.1em}
  {\footnotesize
  \begin{itemize}\itemsep0.28em
    \item \focus{1}{By Cauchy--Schwarz,
          $|\mathbf{d}^{\mathsf T}\nabla E| \leq \|\mathbf{d}\|\,\|\nabla E\|
          = \|\nabla E\|$ for any unit $\mathbf{d}$.}
    \item \focus{2}{So $\mathbf{d}^{\mathsf T}\nabla E \geq -\|\nabla E\|$: no
          unit direction beats $-\|\nabla E\|$.}
    \item \focus{3}{Equality holds only when $\mathbf{d}$ is parallel to
          $\nabla E$; the sign forces \emph{anti}-parallel.}
    \item \focus{4}{So $\mathbf{d}^\star$ attains the bound and is the unique
...
\begin{frame}{Why gradients are worth their cost}
  \bridge{The surface near a minimum is described by $\mathbf{b}$ and $\mathbf{H}$, so
  locating it means pinning those down.}
  \vspace{-0.55em}
  \[
    \underbrace{W}_{\text{entries of } \mathbf{b}}
    \;+\;
    \underbrace{\tfrac{W(W+1)}{2}}_{\text{independent entries of the symmetric } \mathbf{H}}
...
\begin{frame}{The local quadratic model}
  ...
  \vspace{-0.35em}
  \[
    E(\mathbf{w}) \;\simeq\; E(\hat{\mathbf{w}}) + (\mathbf{w} - \hat{\mathbf{w}})^{\mathsf T} \mathbf{b}
          + \tfrac12 (\mathbf{w} - \hat{\mathbf{w}})^{\mathsf T} \mathbf{H} (\mathbf{w} - \hat{\mathbf{w}}),
  \]
  \vspace{-1.0em}
  \[
    \mathbf{b} \equiv \nabla E \big|_{\mathbf{w} = \hat{\mathbf{w}}},
    \qquad
    \mathbf{H}(\hat{\mathbf{w}}) \equiv \nabla\nabla E(\mathbf{w}) \big|_{\mathbf{w} = \hat{\mathbf{w}}} .
  \]
  \vspace{-0.5em}
  \begin{itemize}\itemsep0.35em
    \item $\mathbf{b}$ has length $W$; $\mathbf{H}$ is $W \times W$ and symmetric
    \item At a minimum $\mathbf{w}^\star$ the linear term vanishes, leaving
          $E(\mathbf{w}) = E(\mathbf{w}^\star)
           + \tfrac12 (\mathbf{w}-\mathbf{w}^\star)^{\mathsf T} \mathbf{H} (\mathbf{w}-\mathbf{w}^\star)$
    \item Everything in this section is read off that one expression
  \end{itemize}
  \srcline{Bishop \& Bishop (2024), Eqs.~7.3--7.7.}
\end{frame}

\begin{frame}{Rotating into the eigenvectors of $\mathbf{H}$}
  \vspace{0.1em}
  With $\mathbf{H} \mathbf{u}_i = \lambda_i \mathbf{u}_i$ and $\mathbf{u}_i^{\mathsf T} \mathbf{u}_j = \delta_{ij}$,
  expand the displacement as
  $\mathbf{w} - \mathbf{w}^\star = \sum_i \alpha_i \mathbf{u}_i$. Then
  \vspace{-0.35em}
  \[
  ...
  \vspace{-0.6em}
  \begin{itemize}\itemsep0.35em
    \item A change of coordinates: origin at $\mathbf{w}^\star$, axes along the
          eigenvectors
...
\begin{frame}{Reading the curvature off the contours}
  ...
  \end{center}
  \figcap[0.97]{Contours of the quadratic model are ellipses centred on $\mathbf{w}^\star$, with axes along the eigenvectors $\mathbf{u}_i$ of $\mathbf{H}$ and semi-axis lengths proportional to $\lambda_i^{-1/2}$. Bishop \& Bishop (2024), Fig.~7.2.}
\end{frame}
\begin{frame}{Smoothness: one number for the worst curvature}
  \begin{block}{Definition ($L$-smooth)}
    ...
    \[
      \|\nabla E(\mathbf{u}) - \nabla E(\mathbf{v})\| \;\leq\; L \|\mathbf{u} - \mathbf{v}\|
      \qquad \forall\, \mathbf{u}, \mathbf{v} .
    \]
  \end{block}
  \vspace{0.3em}
  \begin{itemize}\itemsep0.35em
...
\begin{frame}{How far a single step may go}
  ...
  \begin{lemmax}[One step is guaranteed to go downhill]\label{th:desc}
    \small
    Let $E$ be $L$-smooth and let $\mathbf{w}^+ = \mathbf{w} - \eta\,\nabla E(\mathbf{w})$. Then
    \[
      E(\mathbf{w}^+) \;\leq\; E(\mathbf{w})
        - \eta\Big(1 - \tfrac{L\eta}{2}\Big)\,\|\nabla E(\mathbf{w})\|^2 .
    \]
    In particular $E(\mathbf{w}^+) < E(\mathbf{w})$ whenever $0 < \eta < 2/L$ and
    $\nabla E(\mathbf{w}) \neq 0$; the bound is tightest at $\eta = 1/L$.
  \end{lemmax}
  {\footnotesize
  \begin{itemize}\itemsep0.26em
    \item \focus{1}{$L$-smoothness gives $E(\mathbf{v}) \leq E(\mathbf{w}) +
          (\mathbf{v}-\mathbf{w})^{\mathsf T}\nabla E(\mathbf{w}) + \tfrac{L}{2}\|\mathbf{v}-\mathbf{w}\|^2$.}
    \item \focus{2}{Put $\mathbf{v} = \mathbf{w}^+$, so $\mathbf{v} - \mathbf{w} = -\eta\nabla E(\mathbf{w})$:
          $E(\mathbf{w}^+) \leq E(\mathbf{w}) - \eta\|\nabla E\|^2
          + \tfrac{L\eta^2}{2}\|\nabla E\|^2$.}
...
\begin{frame}{Its training set}
  ...
  \end{center}
  \figcap[0.97]{28 points, made by sampling the model at $\mathbf{w} = [0.0,\, 16.6]^{\mathsf T}$ and adding Gaussian noise. Prince (2023), Fig.~6.3.}
\end{frame}
...
\begin{frame}{Stationary points}
  ...
  \begin{propx}[Classification of stationary points]\label{th:sig}
    \small
    Let $\nabla E(\mathbf{w}^\star) = 0$ and let $\mathbf{H}$ be the Hessian at $\mathbf{w}^\star$,
    with eigenvalues $\{\lambda_i\}$. Then $\mathbf{w}^\star$ is a local minimum
  ...
  \begin{itemize}\itemsep0.28em
    \item \focus{1}{Fix $j$, set $\alpha_i = 0$ for $i \neq j$ and vary
          $\alpha_j$: then $E - E(\mathbf{w}^\star) =
          \tfrac12 \lambda_j \alpha_j^2$.}
    \item \focus{2}{So each eigen-direction votes independently: $E$
          rises along $\mathbf{u}_j$ if $\lambda_j > 0$ and falls if
          $\lambda_j < 0$.}
    \item \focus{3}{Writing $\mathbf{v} = \sum_i c_i \mathbf{u}_i$ gives
          $\mathbf{v}^{\mathsf T} \mathbf{H} \mathbf{v} = \sum_i c_i^2 \lambda_i$, so $\mathbf{H}$ is
...
\begin{frame}{The cost of a batch step}
  \vspace{0.3em}
  \begin{itemize}\itemsep0.45em
    \item $E(\mathbf{w}) = \sum_{n=1}^{N} E_n(\mathbf{w})$, so one gradient costs $N$
          backward passes
...
\begin{frame}{Stochastic gradient descent}
  ...
  \[
    \mathbf{w}^{(\tau)} \;=\; \mathbf{w}^{(\tau-1)}
      - \eta\, \nabla E_n\!\left(\mathbf{w}^{(\tau-1)}\right)
  \]
  \vspace{-0.85em}
  \begin{itemize}\itemsep0.3em
  ...
  \begin{algox}[Stochastic gradient descent]\label{th:sgd}
    ...
    error $E_n(\mathbf{w})$; learning rate $\eta$; initial $\mathbf{w}$\\[0.25em]
    $n \leftarrow 1$;\quad
    \textbf{repeat}\quad
      $\mathbf{w} \leftarrow \mathbf{w} - \eta \nabla E_n(\mathbf{w})$;\quad
...
\begin{frame}{The estimate is unbiased}
  ...
  \begin{propx}[Mean and variance of the mini-batch gradient]\label{th:mb}
    ...
    $\hat{\mathbf{g}} = \tfrac{N}{B} \sum_{n \in \mathcal{B}} \nabla E_n(\mathbf{w})$. Then
    $\mathbb{E}[\hat{\mathbf{g}}] = \nabla E(\mathbf{w})$, and each component of $\hat{\mathbf{g}}$
  ...
  \begin{itemize}\itemsep0.28em
    ...
    \item \focus{2}{Linearity over the $B$ draws gives
          $\mathbb{E}[\hat{\mathbf{g}}] = \tfrac{N}{B}\, B\, \tfrac1N \nabla E
          = \nabla E$: \alert{unbiased, for every $B$}.}
    \item \focus{3}{$\hat{\mathbf{g}}$ is $N$ times a sample mean of $B$ draws, so
...
\begin{frame}{Mini-batch stochastic gradient descent}
  ...
  \begin{algox}[Mini-batch stochastic gradient descent]\label{th:msgd}
    ...
    $B$; per-batch error $E_{n:n+B-1}(\mathbf{w})$; learning rate $\eta$;
    initial $\mathbf{w}$\\[0.35em]
    $n \leftarrow 1$\\
    \textbf{repeat}\\
    \hspace{1.4em} $\mathbf{w} \leftarrow \mathbf{w} - \eta \nabla E_{n:n+B-1}(\mathbf{w})$
...
\begin{frame}{One training run, end to end}
  ...
  \begin{tikzpicture}[>=Stealth, line width=0.9pt, font=\small,
      ...
      g/.style={rounded corners=3pt, draw=accc, fill=accc!10,
                align=center, minimum height=0.95cm,
                inner xsep=4pt, font=\footnotesize}]
    \node[g] (init) {initialize $\mathbf{w}$};
    ...
    \node[d, right=5mm of bwd]  (upd) {$\mathbf{w} \leftarrow \mathbf{w} - \eta \nabla E_{\mathcal{B}}$};
...
\begin{frame}{What we choose, and what chooses itself}
  ...
  \begin{tabular}{@{}l@{\hspace{1.6em}}l@{\hspace{1.6em}}l@{}}
    ...
    $\mathbf{w}^{(0)}$ & \alert{us} & taken up in Class 15\\
  \end{tabular}}
  \vspace{0.9em}
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.38em
    ...
    \item $-\nabla E$ is the steepest descent direction, and uniquely so
          (Lemma~\ref{th:steep})
    \item The eigenvalues of $\mathbf{H}$ are curvatures; $\lambda_{\max}$ is the
