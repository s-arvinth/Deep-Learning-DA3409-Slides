...
%  deck with those two removed.  See _shared/PLAN_Classes17_18_19.md.
%
...
\graphicspath{{figs/}{Class18_Regularization_Explicit_Implicit/B_Recreated_Figures/figs/}}
...
\begin{frame}{The flat directions go first}
  \begin{center}
    \includegraphics[width=0.40\linewidth]{shrinkage_factors}
  \end{center}
  \vspace{-0.2em}
  \figcap[0.97]{Each direction's factor $\lambda_i/(\lambda_i+\alpha)$
  against the penalty strength, for the two curvatures of the previous
  figure. A direction is half shrunk at $\alpha = \lambda_i$; the dashed
  sum is the effective number of parameters.}
\end{frame}
