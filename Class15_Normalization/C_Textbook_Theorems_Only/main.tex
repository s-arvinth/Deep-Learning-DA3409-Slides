...
%  in Bishop & Bishop (2024) or Prince (2023).  Four results that the
%  B variant states from the research literature -- the feature-scale
%  law for kappa, batch normalization's scale invariance, the
%  smoothness explanation, and the pre-norm/post-norm placement -- are
%  removed, and what replaces them is unquantified.
%
...
\graphicspath{{figs/}{Class15_Normalization/C_Textbook_Theorems_Only/figs/}}
...
\begin{frame}{Two ways the data alone can wreck the conditioning}
  ...
  $\kappa$ grows sharply with the ratio, and per-feature rescaling
  removes it. (b) two features of equal scale but correlated: $\kappa$
  diverges, and rescaling does nothing.}
\end{frame}
...
\begin{frame}{Why it works is not what was first claimed}
  ...
  \begin{itemize}\itemsep0.42em
    ...
    \item Later work found covariate shift is not the significant
          factor: injecting distribution shift after a normalization
          layer does not undo the benefit
    ...
    \item The next slide measures that smoothness
  \end{itemize}
...
\begin{frame}{A network that trains}
  ...
  \begin{tikzpicture}[>=Stealth, line width=0.9pt, font=\small,
    ...
    \foreach \a/\b in {dn/init, init/fwd, fwd/bwd, bwd/upd}
      \draw[->] (\a) -- (\b);
    \draw[->, dpc] (upd.south) -- ++(0,-0.62)
      -| node[pos=0.25, below, font=\scriptsize, mcMuted]
      {next batch} (fwd.south);
    \node[font=\scriptsize, mcMuted, below=1.2mm of dn]  {once};
    \node[font=\scriptsize, mcMuted, below=1.2mm of init] {once};
    \node[font=\scriptsize, mcMuted, below=1.2mm of fwd] {every step};
  \end{tikzpicture}
  \end{center}
  \vspace{0.4em}
  \begin{itemize}\itemsep0.28em
...
\begin{frame}{Takeaways}
  ...
  \begin{enumerate}\itemsep0.25em
    ...
    \item Rescaling each input fixes the diagonal of $H$, and nothing
          about correlations
    ...
    \item The original explanation, internal covariate shift, has not
          held up; later work points to the smoothness of the error
          surface instead
