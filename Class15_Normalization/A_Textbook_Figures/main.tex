...
\graphicspath{{figs_official/}{Class15_Normalization/A_Textbook_Figures/figs_official/}}
...
\begin{frame}{What normalizing the inputs does}
  \vspace{0.2em}
  \begin{columns}[T, onlytextwidth]
    \begin{column}{0.44\textwidth}
      \begin{center}
        \includegraphics[height=0.60\textheight]{Bishop7_7}
      \end{center}
    \end{column}
    \begin{column}{0.53\textwidth}
      \vspace{0.3em}
      \begin{itemize}\itemsep0.38em
        \item The red circles are the raw data for two variables on
              very different scales
        \item The blue crosses are the same points after each variable
              has been given zero mean and unit variance
        \item The rescaling is computed once, from the training set, and
              the same numbers are applied to validation and test data
      \end{itemize}
      \vspace{0.35em}
      {\scriptsize\color{mcMuted} Bishop \& Bishop (2024), Fig.~7.7.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Fifty layers, five initializations}
  \begin{center}
    \includegraphics[height=0.50\textheight]{Train2Exploding}
  \end{center}
  \figcap[0.97]{Fifty layers of one hundred units at five
  initialization variances. (a) the variance of the activations going
  forward; (b) the variance of the gradients coming back. Only
  $\sigma_w^2 = 2/D = 0.02$ stays level. Prince (2023), Fig.~7.7, where
  this variance is written $\sigma^2_\Omega$.}
\end{frame}
...
\begin{frame}{Which entries share a mean}
  \vspace{0.35em}
  \begin{center}
    \includegraphics[height=0.44\textheight]{Bishop7_8a}\hspace{2.2em}%
    \includegraphics[height=0.44\textheight]{Bishop7_8b}
  \end{center}
  \vspace{0.3em}
  \figcap[0.97]{Left: in batch normalization the mean and variance are computed across the mini-batch, separately for each hidden unit. Right: in layer normalization they are computed across the hidden units, separately for each data point. Bishop \& Bishop (2024), Fig.~7.8.}
\end{frame}
