...
\begin{frame}{What one layer does to the variance}
  ...
  \begin{propx}[Variance across one ReLU layer]\label{th:var}
    ...
    \]
    where $D$ is the fan-in (the number of incoming inputs fed into a single neuron) of the layer.
  \end{propx}
  \vspace{0.05em}
  %{\footnotesize
  % \begin{itemize}\itemsep0.26em
  ...
  %   \item \focus{1}{So $\sigma_{a'}^2 = \mathbb{E}[a_i'^2]
  %         = \sigma_w^2 \sum_j \mathbb{E}[z_j^2]$.}
  %   \item \focus{1}{ReLU zeroes half of a symmetric distribution, giving
  %         $\mathbb{E}[z_j^2] = \tfrac12 \sigma_a^2$.}
  %   \item \focus{1}{Summing over the $D$ inputs gives the result.
  %         \hfill $\square$}
  % \end{itemize}}
  \vspace{-0.4em}
